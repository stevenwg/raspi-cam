...
def image_put(q, user, pwd, ip, channel=1):
    ...
    # #     print('DaHua')
    # if ~cap.isOpened():
    #     cap = cv2.VideoCapture("rtsp://%s:%s@%s/stream0" % (user, pwd, ip))
    #     print('LILIN')
    cap = cv2.VideoCapture("rtsp://%s:%s@%s/stream0" % (user, pwd, ip))
    print('LILIN')
    ...
    while True:
        ...
        q.put(cap.read()[1])
        print("(", ip, "): read time: ", (timeit.default_timer() - start))
        print("(", ip, "): len(q._buffer): ", len(q._buffer))
        q.get() if len(q._buffer) > 1 else time.sleep(0.01)
...
def image_get(q, window_name):
    ...
    while True:
        start = timeit.default_timer()
        frame = q.get()
        print("(", window_name, "): get time: ", (timeit.default_timer() - start))
        cv2.imshow(window_name, frame)
        cv2.waitKey(1)


def run_opencv_camera():
    user, pwd, ip, channel = "admin", "pass", "192.168.1.211", 1
...
def run_single_camera():
    ...
    user_name, user_pwd, camera_ip = "admin", "pass", "192.168.1.211"
...
def run_multi_camera():
    ...
    # ]
    user_name, user_pwd = "osense", "Osense168"
    camera_ip_l = [
        "192.168.1.212",  # ipv4
        "192.168.1.217",  # ipv6
    ]
